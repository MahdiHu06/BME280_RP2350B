...
// helpers 
static void bme_write_register(uint8_t addr, uint8_t reg, uint8_t data);
static void bme_read_bytes(uint8_t addr, uint8_t reg, uint8_t *buffer, size_t length);

// I2C Pin numbers ----EDIT TO WHAT PINS YOU'RE USING //
uint serialData = 0;
uint serialClk = 0;
i2c_inst_t *i2c = NULL;
//------------------------------------//
...
void init_i2c() {
    ...
    gpio_set_function(serialClk, GPIO_FUNC_I2C);

    // pull up pins
    gpio_pull_up(serialData);
    gpio_pull_up(serialClk);
}
...
float calculate_temp(int32_t adc_T, int isFahrenheit) {
    float var1, var2, T;
